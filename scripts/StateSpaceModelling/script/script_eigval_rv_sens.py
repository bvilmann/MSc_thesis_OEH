...
#%%
# for rv in tqdm(np.linspace(0,.1,21)):
for rv in tqdm(np.linspace(0,1,101)):
# for rv in tqdm(np.linspace(0,.01,11)):
...
if not save_: 
    ...
    savefile=f'{plot_path}\\eigval_full_rv_sens_filter.pdf'
else:
    savefile=f'{plot_path}\\eigval_full_rv_sens{suffix}.pdf'

plot_eigvals_dict('$R_v$',sens,save=savefile,y_offset=200,
